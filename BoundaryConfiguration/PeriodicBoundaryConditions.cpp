...
void PeriodicBoundaryConditions::ApplyPeriodicBoundaryConditions(Real x, Real y, Real &x_pbc, Real &y_pbc) const
{
...
void PeriodicBoundaryConditions::ApplyPeriodicBoundaryConditions(std::vector<RealOutput> &system_state)
{
  for (int i : thread_->GetLoopIndices())
  {
    int ii = i * s_;
